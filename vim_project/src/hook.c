...
{
	mlx_string_put(fdf->mlx, fdf->win, 35, 55, fdf->color - 87, "Press ESC to exit");
	mlx_string_put(fdf->mlx, fdf->win, 35, 75, fdf->color - 87, "Move = < ^ v >");
	mlx_string_put(fdf->mlx, fdf->win, 35, 95, fdf->color - 87, "Zoom = + -");
	mlx_string_put(fdf->mlx, fdf->win, 35, 115, fdf->color - 87, "Change height = 8 5");
	mlx_string_put(fdf->mlx, fdf->win, 35, 135, fdf->color - 87, "Rotate z = Q A");
	mlx_string_put(fdf->mlx, fdf->win, 35, 155, fdf->color - 87, "Rotate x = E D");
	mlx_string_put(fdf->mlx, fdf->win, 35, 175, fdf->color - 87, "Rotate y = W S");
	mlx_string_put(fdf->mlx, fdf->win, 35, 195, fdf->color - 87, "Change color = C");
}
